...
LRESULT WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
		...
		_ptMouse.x = (float)LOWORD(lParam) * (windowRc.right - windowRc.left) / (windowClnt.right - windowClnt.left);
		_ptMouse.y = (float)HIWORD(lParam) * (windowRc.bottom - windowRc.top) / (windowClnt.bottom - windowClnt.top);
