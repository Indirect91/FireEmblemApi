...
private:
	typedef std::unordered_map<std::wstring, IDWriteTextFormat*>::iterator TextIter;
private:
	...
	std::unordered_map<std::wstring, IDWriteTextFormat*> mFontList;
public:
	D2DRenderer();
	virtual ~D2DRenderer();
public:
...
public:
	...
	void RenderText( int x,  int y, std::wstring text,  int size,
		...
		std::wstring font = L"맑은고딕");
	//텍스트 출력 (원하는 색상과 알파값으로 브러쉬 생성해서 출력)
	void RenderText( int x,  int y, std::wstring text,  COLORREF color,  float alpha,  int size,
		 DWRITE_TEXT_ALIGNMENT align = DWRITE_TEXT_ALIGNMENT_LEADING,  
		std::wstring font = L"맑은고딕");
	//텍스트 필드 출력 (기본 브러쉬 사용) 
	void RenderTextField( int x,  int y, std::wstring text,  int size,  int width,  int height,
		 DefaultBrush defaultBrush = DefaultBrush::Black,
		 DWRITE_TEXT_ALIGNMENT align = DWRITE_TEXT_ALIGNMENT_LEADING, 
		std::wstring font = L"맑은고딕");
	//텍스트 필드 출려(원하는 색상과 알파값으로 브러쉬 생성해서 출력)
	void RenderTextField( int x,  int y, std::wstring text,  COLORREF color,
